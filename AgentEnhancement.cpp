...
void AgentEnhancement::solveMaze() {
    ...
    std::vector<mcpp::Coordinate> directions = {
        currentLoc + MOVE_XPLUS,
        currentLoc + MOVE_XMINUS,
        currentLoc + MOVE_ZPLUS,
        currentLoc + MOVE_ZMINUS
    };
    ...
    // X positive
    for (unsigned int z = 0; z < mazeNodes.size() && (blueCarpetFound == false); z++) {
    ...
    if (mc.getBlock(currentLoc) == mcpp::Blocks::BLUE_CARPET) {
        ...
        for (size_t m = 0; m < shortestPath.size(); m++) {
            ...
            std::cout << "Step[" << numberOfSteps << "]: " << shortestPath[m] << std::endl;
            ++numberOfSteps;
        }
    }
